...
export default function CoopLandingStandalone() {
  ...
  const [roofMat, setRoofMat] = useState("wood");
  ...
  // colour presets for cedar vs pine
    ...
    <main style={styles.main}>
      ...
      <header className="top">
        <div className="wrap bar">
          <div className="logo">
            ...
            <div>
              COOPS<span style={{ color: "var(--brand)", margin: "0 .25rem" }}>/</span>USA
            </div>
          </div>
          <nav className="nav">
      ...
      <section id="build" className="section">
        <div className="wrap grid hero" style={{ padding: "56px 20px 72px" }}>
          ...
          <div>
            ...
            <div className="controls" style={{ marginTop: 24, display: "grid", gap: 20 }}>
              {/* wood selection */}
              <div>
                <label>Wood</label>
                <div className="row">
                ...
                <div className="row">
                  {["wood", "metal"].map((m) => (
                    <button
                      key={m}
                      type="button"
                      onClick={() => setRoofMat(m)}
                      className={"toggle" + (roofMat === m ? " active" : "")}
                    >
                ...
                <div className="row">
                  {[1, 2].map((n) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setLevels(n)}
                      className={"toggle" + (levels === n ? " active" : "")}
                    >
                      {n} LEVEL{n === 1 ? "" : "S"}
                    </button>
                  ))}
                ...
                <div className="row">
                  {[false, true].map((v) => (
                    <button
                      key={v ? "yes" : "no"}
                      type="button"
                      onClick={() => setTimedDoor(v)}
                      className={"toggle" + (timedDoor === v ? " active" : "")}
                    >
                      {v ? "YES" : "NO"}
                    </button>
                  ))}
              ...
              <div className="price">
                ...
                <div style={{ font: "700 26px/1 ui-sans-serif" }}>
      ...
      <section id="proof" className="section">
        <div className="wrap" style={{ padding: "72px 20px" }}>
          <div className="grid cards">
            ...
            <ProofCard title="Hardware Cloth" subtitle="1/2″ mesh, wrapped & stapled">
              <MeshSVG />
            </ProofCard>
            <ProofCard title="Vent & Clean-out" subtitle="Dry interior, easy to service">
              <VentSVG />
            </ProofCard>
          </div>
        </div>
          ...
          <div className="grid specs">
            <div>
              ...
              >
                <li>
                  <span className="chip">
                    <span className="ic" /> Cedar / Pine
                  </span>
                </li>
                <li>
                  <span className="chip">
                    <span className="ic" /> Predator Latches
                  </span>
                </li>
                <li>
                  <span className="chip">
                    <span className="ic" /> Custom Dimensions
                  </span>
                </li>
                <li>
                  <span className="chip">
                    <span className="ic" /> Hand Finished
                  </span>
                </li>
              </ul>
            </div>
      ...
      <section id="quote" className="section">
        <div className="wrap" style={{ padding: "72px 20px" }}>
          ...
          <p className="muted" style={{ marginTop: 6 }}>
            Zero obligation. Send your rough config — we’ll reply with a dialed proposal.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            ...
            >
              ...
              <button
                className="primary"
                style={{ border: "none", borderRadius: 14, padding: "12px 18px", cursor: "pointer" }}
              >
                Send
              </button>
            </div>
      ...
      <div className="mobile-sticky">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
          <a
            className="primary"
            href={mailtoQuote({ wood, roof, lengthFt, hardware, levels, roofMat, timedDoor })}
          >
            Build
          </a>
...
function ProofCard({ title, subtitle, children }) {
  return (
    <div className="card">
      ...
      <div className="card" style={{ padding: 12, height: 180 }}>
        {children}
      </div>
    </div>
  );
...
function VentSVG() {
  return (
    <svg viewBox="0 0 300 160" width="100%" height="100%">
      ...
      <path
        d="M20 120 C80 90,120 150,180 120 S260 140,290 110"
        stroke="#7dd3fc"
        strokeWidth="2"
        fill="none"
        opacity=".7"
    ...
    <div className="specgrid">
      {items.map((it) => (
        <div key={it.k} className="spec">
          <div
            className="muted"
            style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: ".18em" }}
          >
            {it.k}
          </div>
...
function CoopSVG({
  ...
  // choose units
  const unit = 20;
  const width = Math.max(4, Math.min(16, lengthFt)) * unit;
  const height = 6 * unit;
  const runHeight = height * 0.5;
  const roofHeight = unit * (roof === "gable" ? 1.5 : 1.0);
  const eave = unit * 0.6;

  // helper to shade a hex colour
  function shade(hex, amt) {
    const x = hex.slice(1);
    const r = parseInt(x.slice(0, 2), 16),
      g = parseInt(x.slice(2, 4), 16),
      b = parseInt(x.slice(4, 6), 16);
    const adjust = (v) => Math.max(0, Math.min(255, Math.round(v + 255 * amt)));
    return (
      "#" +
  ...
  // roof points: gable vs lean-to
  const roofPoints =
    roof === "gable"
      ? `${-eave},-${roofHeight} ${width / 2},-${roofHeight * 1.4} ${
          width + eave
        },-${roofHeight}`
      : `${-eave},-${roofHeight} ${width + eave},0`;

  return (
    <svg
      viewBox={`0 ${-roofHeight} ${width + eave * 2} ${height + roofHeight}`}
      style={{ width: "100%", height: "auto" }}
    >
      {/* roof */}
      {roof === "gable" ? (
        <polygon
          points={roofPoints + ` ${width},0 0,0`}
          fill={roofMat === "metal" ? "#BBC4D6" : shade(woodColors.mid, -0.18)}
      ...
      <rect
        ...
        strokeWidth="2"
      />
      <pattern id="meshPat2" width="8" height="8" patternUnits="userSpaceOnUse">
        <path d="M0 0 L8 0 M0 0 L0 8" stroke={meshStroke} strokeWidth="1" />
        <path
          d="M8 0 L8 8 M0 8 L8 8"
          stroke={meshStroke}
          strokeWidth="1"
          opacity="0.35"
        />
      ...
      <circle
        cx={unit * 3}
        cy={height - runHeight + unit * 2.1}
        r={3}
        fill={meshStroke}
      />
      {/* timed door badge */}
      {timedDoor && (
        <g transform={`translate(${unit * 0.8},${height - runHeight + unit * 0.3})`} opacity="0.9">
          <rect width="20" height="14" rx="3" fill="#0b0b0c" stroke="#666" />
          <path d="M6 4 L6 8 L10 8" stroke="#fbbf24" strokeWidth="2" fill="none" />
        </g>
      )}

      {/* window near top right */}
      <rect
        x={width - unit * 3.4}
        y={unit}
        width={unit * 2.4}
      ...
      <rect
        ...
        opacity=".4"
      />
      {/* if two levels: roost and ladder */}
      {levels === 2 && (
        <>
          {/* roost platform on right with arch */}
          <rect
            x={width - width * 0.55 - unit * 0.5}
            y={height * 0.2}
            width={width * 0.55}
            height={height * 0.18}
            fill={shade(woodColors.base, -0.04)}
            stroke={woodColors.dark}
            strokeWidth="2"
          />
          {/* arch opening */}
          <path
            d={`
              M ${width * 0.7} ${height * 0.2}
              L ${width * 0.88} ${height * 0.2}
              A ${width * 0.09} ${height * 0.15} 0 0 1 ${width * 0.88} ${
              height * 0.38
            }
              L ${width * 0.7} ${height * 0.38} Z
            `}
            fill={shade(woodColors.mid, -0.1)}
            stroke={woodColors.dark}
            strokeWidth="2"
          />
          {/* ladder from run to roost */}
          <line
            x1={width * 0.2}
            y1={height - runHeight}
            x2={width * 0.65}
            y2={height * 0.2 + height * 0.18}
            stroke={woodColors.dark}
            strokeWidth="4"
          />
          {Array.from({ length: 6 }).map((_, i) => (
            <line
              key={i}
              x1={width * 0.2 + (width * 0.45) * (i / 6)}
              y1={height - runHeight - (height * 0.32) * (i / 6)}
              x2={width * 0.23 + (width * 0.45) * (i / 6)}
              y2={height - runHeight - (height * 0.32) * (i / 6) + unit * 0.1}
              stroke={woodColors.dark}
              strokeWidth="4"
            />
          ))}
...
function mailtoQuote({
  ...
  const subject = encodeURIComponent(
    `Custom Coop Quote — ${wood} · ${roof}/${roofMat} · ${lengthFt}ft · ${levels} level${
      levels === 1 ? "" : "s"
    }${timedDoor ? " · timed door" : ""}`
  );
